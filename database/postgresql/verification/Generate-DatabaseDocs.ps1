...
[void]$relationDocument.AppendLine('> 本文件由 Migration 的 Column Comment 生成，登记数据库可识别的逻辑引用及可静态推导的必填性。关系类型、租户守卫、删除行为、锁和校验 Owner 必须按 `docs/架构决策/架构决策记录.md#adr-0001` 与命令规格卡完成；未分类关系阻断数据库基线最终冻结。')
[void]$relationDocument.AppendLine()
