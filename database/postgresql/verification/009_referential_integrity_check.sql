...
BEGIN
    ...
    IF foreign_key_count <> 187 OR invalid_action IS NOT NULL OR invalid_validation IS NOT NULL OR missing_critical IS NOT NULL THEN
...
SELECT 'PASS: 187 个直接内部引用由已验证 RESTRICT 外键保护；多态、数组和外部引用保留代码校验' AS result;
