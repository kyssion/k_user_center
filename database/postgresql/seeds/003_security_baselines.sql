...
RETURNING config_type, config_code
), matched AS (
SELECT seed.config_type, seed.config_code
FROM seed
WHERE EXISTS (
          SELECT 1
          FROM applied
          WHERE applied.config_type = seed.config_type
            AND applied.config_code = seed.config_code
      )
   OR EXISTS (
       SELECT 1
       FROM iam.configuration_versions current_version
       WHERE current_version.config_type = seed.config_type
