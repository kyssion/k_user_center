...
| `逻辑关系与代码校验清单.md` | 无 Foreign Key 时的逻辑引用登记和非数据库校验提示 |
| `需求能力表测试追踪矩阵.md` | 能力地图/蓝图编号到数据库持久化边界的覆盖索引；不是代码实施矩阵 |
...
- 非数据库职责：引用存在性、租户一致性、状态转换、权限、风险、审批、加密、协议、幂等语义、流程和删除编排；具体实现以后续代码实施文档为准。
...
3. 执行生成脚本，确认 718 个数据库需求覆盖编号、完整逻辑关系和数据库报告全部更新。
