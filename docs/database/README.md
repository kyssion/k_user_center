...
- 当前候选 SQL 不创建 Foreign Key、业务 Trigger、持久化业务 Routine、PostgreSQL Enum 和业务 View。无 Foreign Key 策略在最终冻结前必须按 [`ADR-0001`](../架构决策/ADR-0001-逻辑关系与外键策略.md) 完成逐关系分类；这不改变当前 SQL 的可执行权威地位。
