...
本目录只描述业务模型是否能被数据库事实完整表达、哪些关系需要持久化以及 PostgreSQL 应承担哪些硬边界。代码目录、类、接口、Handler、Repository、API DTO 和外部组件调用顺序不在本目录定义；命令、事务、权限和校验边界由 [`docs/implementation`](../implementation/README.md) 展开。
...
| `../implementation/*.md` | 参数、逻辑引用、业务权限、事务、状态机、幂等和错误映射的代码实施契约 |
...
- 非数据库职责：引用存在性、租户一致性、状态转换、最终用户/管理员授权、数据范围、领域 Owner、风险、审批、加密、协议、幂等语义、流程和删除编排；具体契约见 [`docs/implementation`](../implementation/README.md)。
...
4. 按 [`docs/implementation`](../implementation/README.md) 执行参数/Schema、状态机、逻辑引用、业务授权/数据范围、领域 Owner、租户隔离、追加/不可变历史篡改和敏感日志测试。
