...
- `migration.authority_lease`
- `migration.legacy_id_mapping`
- `migration.duplicate_candidate`
- `migration.change_log`
- `migration.reconciliation_run`
- `migration.rollback_execution`

## 依赖与执行

- 前置：先执行 `../../bootstrap.sql`。
- 局部构建：在本目录执行 `psql --file build.sql`。
- 跨域依赖：`control`；所有 Schema 的 `build.sql` 完成后再执行本模块 `links.sql`。
