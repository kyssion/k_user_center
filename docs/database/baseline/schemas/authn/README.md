...
认证域：认证器、密码、Challenge、Login Transaction、设备授权与保证等级证据。
...
- `authn.authenticator`
- `authn.password_credential`
- `authn.password_history`
- `authn.recovery_code_batch`
- `authn.recovery_code`
- `authn.login_transaction`
- `authn.login_factor`
- `authn.verification_challenge`
- `authn.device_authorization`

## 依赖与执行

- 前置：先执行 `../../bootstrap.sql`。
- 局部构建：在本目录执行 `psql --file build.sql`。
- 跨域依赖：`iam`、`oauth`、`org`、`risk`；所有 Schema 的 `build.sql` 完成后再执行本模块 `links.sql`。
