...
机器主体、工作负载证明、机器凭证与 Token Exchange。
...
- `workload.machine_principal`
- `workload.machine_credential`
- `workload.trust_bundle`
- `workload.workload_attestation`
...
- 跨域依赖：`assurance`、`control`、`crypto`、`org`；所有 Schema 的 `build.sql` 完成后再执行本模块 `links.sql`。
