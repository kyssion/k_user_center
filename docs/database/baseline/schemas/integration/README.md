...
事件 Schema、Outbox、Webhook、投递、回放与消费方水位。
...
- 跨域依赖：`control`、`oauth`、`org`、`privacy`；所有 Schema 的 `build.sql` 完成后再执行本模块 `links.sql`。
