...
OAuth/OIDC 会话与授权域：Client、Resource、Grant、Session、Token、撤销与退出。
...
- `oauth.client`
- `oauth.client_uri`
- `oauth.client_credential`
- `oauth.api_resource`
- `oauth.scope_definition`
- `oauth.device`
...
- `oauth.reference_access_token`
- `oauth.revocation_record`
- `oauth.logout_request`
- `oauth.logout_target_result`

## 依赖与执行

- 前置：先执行 `../../bootstrap.sql`。
- 局部构建：在本目录执行 `psql --file build.sql`。
- 跨域依赖：`authn`、`control`、`iam`、`org`、`privacy`；所有 Schema 的 `build.sql` 完成后再执行本模块 `links.sql`。
