...
身份主体与标识域：Global User、pairwise Subject、Identifier、账号合并与注销。
...
- `iam.user_account`
- `iam.subject_assignment`
- `iam.identifier`
- `iam.identifier_tombstone`
- `iam.account_merge`
- `iam.account_merge_item`
- `iam.account_deletion`

## 依赖与执行

- 前置：先执行 `../../bootstrap.sql`。
- 局部构建：在本目录执行 `psql --file build.sql`。
- 跨域依赖：`control`、`oauth`；所有 Schema 的 `build.sql` 完成后再执行本模块 `links.sql`。
