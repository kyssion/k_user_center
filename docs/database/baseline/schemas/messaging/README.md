...
短信、邮件、Push 等消息供应商、模板、发送、回执与可达性。
...
- `messaging.provider`
- `messaging.route_policy`
- `messaging.message_template`
- `messaging.message_send`
- `messaging.delivery_receipt`
- `messaging.reachability`
- `messaging.content_compliance_rule`
- `messaging.provider_metric`

## 依赖与执行

- 前置：先执行 `../../bootstrap.sql`。
- 局部构建：在本目录执行 `psql --file build.sql`。
- 跨域依赖：`authn`、`control`、`iam`、`oauth`、`org`；所有 Schema 的 `build.sql` 完成后再执行本模块 `links.sql`。
