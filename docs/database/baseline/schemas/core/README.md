...
平台公共契约：迁移、幂等、异步操作、参考数据与数据库辅助函数。

## 文件

- `tables.sql`：本模块除迁移台账外的全部基表、同 Schema 约束、索引和对象注释；迁移台账由 bootstrap 创建。
...
## 表清单（10；其中迁移台账由 bootstrap 创建）

- `core.schema_migration`（由 `../../bootstrap.sql` 创建）
- `core.public_id_ledger`
- `core.security_profile`
- `core.duration_policy`
- `core.data_classification`
- `core.error_registry`
- `core.requirement_trace`
- `core.async_operation`
- `core.async_operation_step`
- `core.idempotency_request`

## 依赖与执行

- 前置：先执行 `../../bootstrap.sql`。
- 局部构建：在本目录执行 `psql --file build.sql`。
- 对象权限：完成全库对象和 `../../roles.sql` 后，在本目录执行 `psql --file security.sql`。
- 跨域依赖：`control`、`org`；所有 Schema 的 `build.sql` 完成后再执行本模块 `links.sql`。
- 种子：跨域绑定完成后执行 `seed.sql`。
