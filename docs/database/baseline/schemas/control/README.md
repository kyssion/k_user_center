...
配置发布、高风险审批、安全例外、Break-glass 与复核。
...
- `control.approval_case`
- `control.approval_decision`
- `control.config_release`
- `control.security_exception`
- `control.break_glass_grant`
- `control.owner_review`
- `control.client_certification_run`

## 依赖与执行

- 前置：先执行 `../../bootstrap.sql`。
- 局部构建：在本目录执行 `psql --file build.sql`。
- 跨域依赖：`iam`、`oauth`、`org`；所有 Schema 的 `build.sql` 完成后再执行本模块 `links.sql`。
