...
IAL/AAL/FAL、敏感操作要求、高保证恢复与自然人委托。
...
- `assurance.identity_assurance_assertion`
- `assurance.recovery_request`
...
- 跨域依赖：`control`、`iam`、`org`、`risk`；所有 Schema 的 `build.sql` 完成后再执行本模块 `links.sql`。
