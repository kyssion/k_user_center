...
SELECT (NOT EXISTS (SELECT 1 FROM core.schema_migration WHERE version = 'baseline:rls'))::text AS kuc_run_rls \gset
\if :kuc_run_rls
-- =============================================================================
-- baseline/rls_optional.sql
...
BEGIN
    ...
    LOOP
        ...
        EXECUTE format(
            'COMMENT ON POLICY tenant_context_policy ON %I.%I IS %L',
            r.schema_name, r.table_name,
            '租户数据角色仅能访问和写入当前事务 app.tenant_id 对应的记录。'
        );
        EXECUTE format(
            'COMMENT ON POLICY platform_control_policy ON %I.%I IS %L',
            r.schema_name, r.table_name,
            '所有者、迁移、控制与审计角色的受控平台范围访问策略。'
        );
    END LOOP;
END;
$$;
...
CREATE POLICY tenant_root_platform_policy ON org.tenant
    TO kuc_owner, kuc_control_writer, kuc_audit_writer, kuc_auditor, kuc_migrator
    USING (true) WITH CHECK (true);
COMMENT ON POLICY tenant_root_context_policy ON org.tenant IS '租户数据角色仅能访问当前事务 app.tenant_id 对应的租户根记录。';
COMMENT ON POLICY tenant_root_platform_policy ON org.tenant IS '所有者、迁移、控制与审计角色的受控租户根平台访问策略。';
...
CREATE POLICY business_line_platform_policy ON org.business_line
    TO kuc_owner, kuc_control_writer, kuc_audit_writer, kuc_auditor, kuc_migrator
    USING (true) WITH CHECK (true);
COMMENT ON POLICY business_line_context_policy ON org.business_line IS '租户数据角色仅能访问当前租户所属的业务线。';
COMMENT ON POLICY business_line_platform_policy ON org.business_line IS '所有者、迁移、控制与审计角色的受控业务线平台访问策略。';
...
COMMENT ON POLICY tenant_context_policy ON authz.role IS '租户数据角色仅能访问当前租户或其业务线范围内的授权对象。';
...
COMMENT ON POLICY tenant_context_policy ON authz.role_assignment IS '租户数据角色仅能访问当前租户或其业务线范围内的授权对象。';
...
BEGIN
    ...
    LOOP
        ...
        EXECUTE format(
            ...
            'TO kuc_owner, kuc_control_writer, kuc_audit_writer, kuc_migrator, kuc_auditor '
            'USING (true) WITH CHECK (true)',
            r.table_name
        );
        EXECUTE format(
            'COMMENT ON POLICY derived_tenant_context_policy ON %s IS %L',
            r.table_name,
            '通过受控父对象关系推导当前 app.tenant_id 的租户隔离策略。'
        );
        EXECUTE format(
            'COMMENT ON POLICY derived_platform_control_policy ON %s IS %L',
            r.table_name,
            '所有者、迁移、控制与审计角色的受控派生租户平台访问策略。'
        );
    END LOOP;
END;
$$;
...
SELECT core.fn_register_migration('baseline:rls', '可选 PostgreSQL RLS：直接与派生租户隔离策略');
COMMIT;
\endif
