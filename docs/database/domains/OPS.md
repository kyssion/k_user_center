...
- 持久化范围：`operations`、`operation_steps`、`operation_policy_versions`、`approval_cases`、`audit_events`。
...
- 业务模型要求：`operations + operation_steps` 只保存检查点、幂等步骤状态、不可逆边界、超时、结果和证据，`operation_policy_versions` 保存创建时实际采用的策略版本有序集合；策略选择、补偿、前向修复与人工接管流程不在数据库中实现。
