...
> 文档版本：v2.4
>
> 设计来源：`docs/能力地图.md` v1.2、`docs/统一身份与访问平台建设与验收蓝图.md` v1.2
>
...
本基线包含 18 个业务 Schema、145 张基表、3 个视图、11 个默认迁移版本，以及覆盖数据库、扩展、Schema、表/视图/序列、列、Type/Domain、索引、约束、触发器、函数/过程和角色的完整注释生成/验收机制；启用 RLS 后同样覆盖 Policy。
...
| 8 | `migrations/070_migration_catalog_comments.sql` | 旧系统迁移、双轨权威、反向 CDC、对账、全维度对象注释与数据字典 |
| 9 | `migrations/075_integrity_hardening.sql` | 审批绑定、上下文一致性、蓝图状态机、不可变证据、组织/角色守卫和全部外键前导索引 |
...
完整字段、约束和描述以 SQL 中的显式 `COMMENT`、迁移补齐函数、`core.data_dictionary` 与 `core.object_dictionary` 为准。重要安全列使用专用描述，其他列和对象使用其所属领域、对象类型及治理目的生成非空说明；`verify.sql` 会拒绝空注释和旧版占位说明：
...
ORDER BY object_dimension, schema_name, parent_object, object_name;
```
...
| 维度 | 实现 |
|---|---|
| Database | `000_bootstrap.sql` 对 `current_database()` 执行 `COMMENT ON DATABASE` |
| Extension | `COMMENT ON EXTENSION pgcrypto` |
| Schema | 18 个领域 Schema 显式描述，补齐函数兜底 |
| Table / View / Materialized View / Sequence | 表与视图显式描述；当前无业务序列，后续新增时由补齐函数和验收自动覆盖 |
| Column | 基表、视图和物化视图列均生成语义化描述 |
| Type / Domain | 当前不创建独立 Type/Domain；后续新增时自动补齐并进入对象字典与验收 |
| Index | 主键、唯一、部分及查询索引逐项描述 |
| Constraint | 主键、唯一、外键、CHECK、排斥约束逐项描述 |
| Trigger | 每个非内部触发器描述所属表、调用函数及治理目的 |
| Function / Procedure | 所有平台函数均有描述和最小权限约束；后续新增 Procedure 同样自动覆盖 |
| Role | 10 个 `kuc_*` NOLOGIN 角色逐项说明职责 |
| Policy | 执行可选 `910` 后为每个 RLS Policy 补齐说明 |

仅对平台迁移实际创建或未来可能在 18 个平台 Schema 内创建的用户对象负责；PostgreSQL 内部对象以及本基线未创建的 Tablespace、FDW/Server、Publication/Subscription、Language、Operator 等不伪造平台 COMMENT。
...
| Login Transaction 不得跳步或改写请求上下文 | 强制 `CREATED → IDENTIFIED → PARTIALLY_AUTHENTICATED/AUTHENTICATED → PENDING_CONSENT/COMPLETED`，未完成态只能进入明确终态；状态时间由数据库生成 |
| Challenge 不得跨流程、跨 Client 或重复消费 | Challenge 强制绑定目标、Login Transaction、Risk Assessment 与风险摘要；只允许 `ISSUED → VERIFIED → CONSUMED` 或明确终态，尝试序号单调 |
| Session 不得伪造初态、原因或终态时间 | Session 强制从 ACTIVE 创建，可过期、直接撤销或先失陷再正式撤销；原因与时间由数据库生成并冻结 |
...
| Tenant、Membership、Invitation 不得跳过蓝图状态 | 领域状态守卫强制初态、合法边和数据库状态时间；Tenant 状态变化推进 tenant security epoch，Invitation 创建/接受分别绑定新鲜 ALLOW 决策，接受时重新校验邀请目标、邀请人、角色和 Membership 范围 |
| Business Line、Organization 不得绕过父子范围关闭规则 | Business Line 暂停/关闭前先收敛下属 Tenant，关闭证据越过不可逆边界后不得撤回；Organization 关闭前必须清理有效子组织和 Membership |
...
| 机器主体暂停恢复与证明签发不得复用旧信任 | Machine reactivation 必须提交新的复核证据摘要并推进 epoch；Attestation 逐次校验活动主体、Trust Bundle、环境、issuer、audience、时间窗口和一次性 jti |
| 机器凭证材料、轮换和替代链不得混用 | 按 `PRIVATE_KEY_JWT/MTLS/WORKLOAD_FEDERATION/SECRET` 强制材料组合；替代凭证唯一关联旧凭证，激活前旧凭证必须处于 ROTATING，签名键或 mTLS 证书须同环境且有效 |
| Trust Bundle、Certificate、JWKS 不得跳过密码学发布窗口 | Trust Bundle 的规范化内容摘要由数据库重算并绑定已执行 `KEY_OPERATION` 审批；Certificate 校验密钥与有效期；JWKS 只接受同环境 `TOKEN_SIGNING` 键并等待至少 `2 × cache_max_age` 后激活 |
| 委托、安全例外和 Break-glass 不得绕过审批/期限 | Delegation 父链动作、资源和深度不得扩张；安全例外与 Break-glass 使用数据库重算的不可变上下文摘要，并且只在批准/激活转换中写入已执行审批绑定，状态时间由数据库生成 |
...
- 蓝图明确的核心状态机使用领域专用触发器同时保护初态、合法转换、状态时间、原因和不可变上下文；简单终态另由 `core.fn_terminal_state_guard` 兜底。
...
- Delegation、Trust Bundle、安全例外和 Break-glass 的审批摘要不是任意客户端字段：数据库使用 `core.fn_hash_jsonb(jsonb_build_object(...))` 对约定字段重算 SHA-256；集合型代码先排序、二进制值转十六进制、时间转 UTC epoch 秒，应用与审批服务必须使用相同字段集合和 JSONB 规范化规则。
...
3. 首次空库引导必须使用数据库 owner 或具备等价受控权限、可创建/描述 `pgcrypto`、可创建角色并可向 `kuc_owner` 转移所有权的引导账户；否则数据库级或扩展级 COMMENT、角色创建和所有权转移会失败。常规迁移登录角色必须属于 `kuc_migrator`。`080_roles_permissions.sql` 转移所有权后，`build.sql` 会自动 `SET ROLE kuc_owner` 再执行种子和验收，后续独立迁移也必须先显式切换角色。
...
| 角色 | 目的 |
|---|---|
| `kuc_owner` | 所有平台 Schema、表、视图、索引、约束、触发器和函数的统一 NOLOGIN owner；仅供迁移 `SET ROLE` |
...
- 当前 Database、`pgcrypto`、18 个 Schema、全部表/视图、列、索引、约束、非内部触发器、函数和 10 个角色是否有非空描述；启用 910 后检查全部 RLS Policy 描述。
- 每张基表是否有主键，每个基表/视图列是否有非空且非旧占位描述。
