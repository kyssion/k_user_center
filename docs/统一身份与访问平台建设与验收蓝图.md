...
> 文档状态：规范基线 v1.2（已与能力地图和 PostgreSQL 基线交叉核对；项目级 Owner、交付阶段与证据矩阵须在阶段 0 实例化）
...
| INV-G-016 | Login Transaction 未达 `COMPLETED` 不得签发任何 Session、Token 或授权码，其标识不得被当作会话标识使用 | 状态机属性测试 + 协议负向测试 |
| INV-G-017 | 审批单发起人不得同时是审批人；审批通过后的执行必须绑定该单据且仅可执行一次 | 职责分离测试 |
