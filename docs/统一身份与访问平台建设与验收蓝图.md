...
> 文档状态：规范基线 v1.1（已与能力地图交叉核对；项目级 Owner、交付阶段与证据矩阵须在阶段 0 实例化）
...
- `business_line_state`、`organization_state`：业务线与组织，沿用 `tenant_state` 的状态集与转换语义，见 §10.1。
- `privacy_request_state`：个人权利请求，见 §11.1。
- `client_state`、`machine_principal_state`、`attestation_state`：Client、机器主体与工作负载证明，见 §13.1。
...
本清单是平台可变状态字段的完整登记表：能力地图 §3.1 中具有生命周期或可变业务状态的核心对象必须落到上表的一个或多个正交状态字段；不可变事件、短期签名凭证和值对象使用版本、有效期或完整性规则治理，不得为满足形式要求虚构状态。新增可变状态必须同时在此登记并在对应章节定义合法转换，否则视为违反能力地图原则 12。
...
`risk_state` 表达单次不可变风险评估结果，取值为 `UNKNOWN/LOW/MEDIUM/HIGH/CRITICAL`；处置结果独立为 `ALLOW/CHALLENGE/STEP_UP/WAIT/REVIEW/DENY/FREEZE`。风险重新计算必须创建新评估或新版本，不得原地改写历史证据；处置状态不得覆盖用户生命周期、安全冻结或 Membership 状态。
