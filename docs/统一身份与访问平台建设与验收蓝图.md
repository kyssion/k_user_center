...
- PostgreSQL 负责保存权威事实、逻辑关系标识、PK/Unique/NOT NULL/基础 CHECK、明确内部引用的 `RESTRICT` Foreign Key、内部多值关系表、`row_version`、安全水位、不可变证据、技术时间、访问控制和必须原子完成的一次性消费边界；多态、外部引用及业务有效性由代码校验。
- 完整状态机、跨对象业务有效性、Tenant/Client/User/Machine/Consent/Approval/Risk 上下文匹配、权限、风险、审批、协议处理和流程副作用属于非数据库职责，不得通过业务 Trigger、Routine、Enum 或业务 View 在 PostgreSQL 中实现第二套领域逻辑；唯一例外是自动维护 `updated_at/row_version` 的纯技术 Trigger。
...
- `AT-AUTHZ-010`：角色、权限、业务线/租户作用域与数据范围的组合矩阵只产生预期允许结果；删除匹配条件或篡改任一作用域后默认拒绝。
