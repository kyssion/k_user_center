...
- `device_state`：设备实体的生命周期与可信维度，独立于 Device Session，见 §9.1。
...
本清单是状态字段的完整登记表：能力地图 §3.1 的每个核心对象都必须落到上表的某一个状态字段，新增核心对象必须同时在此登记并在对应章节定义合法转换，否则视为违反能力地图原则 12。
...
`DELETION_PENDING → DELETION_BLOCKED`（法律保留或业务阻断）  
...
`CREATED → IDENTIFIED → PARTIALLY_AUTHENTICATED → PENDING_CONSENT → COMPLETED`，以及 `→ EXPIRED/ABANDONED/BLOCKED`
...
Device 生命周期：`REGISTERED → RETIRED/REVOKED`；可信维度独立为 `UNTRUSTED ⇄ TRUSTED`；挂失进入 `LOST` 并撤销该设备的全部 Session、Refresh Token Family 与可信标记。设备状态不随单个 Device Session 结束而改变。
...
- `REQ-SESSION-012 MUST`：资源服务器通过短时 Access Token、内省或签名撤销流持续获取 user/client/tenant security epoch 与 revocation watermark；缓存刷新上界不得超过适用撤销 SLO（SP2/SP3/SP5 与安全事件类变更适用 `SLO-REVOKE-001`，SP1 的非安全类授权变更适用 `SLO-REVOKE-002`）。
...
2. 能力地图中每个 **P0 能力编号**至少被一行的 `capability_id` 覆盖；缺行即阻断发布。
3. 适用测试最近一次通过，且例外未过期。
...
| `REQ-OAP-*` | Client Profile、协议元数据 | OIDC/OAuth 一致性与负向套件 | 身份协议团队 |
