...
> 文档状态：规范基线 v1.3（已与能力地图和 PostgreSQL 基线交叉核对；项目级 Owner、交付阶段与证据矩阵须在阶段 0 实例化）
...
- .NET 领域服务是完整状态机和跨表业务规则的唯一执行入口：负责初态、合法边、前置条件、状态原因/时间、Tenant/Client/User/Machine/Consent/Approval/Risk 上下文匹配以及同事务副作用。
- PostgreSQL 负责 PK/FK/NOT NULL/CHECK、唯一性、`row_version`、epoch 不回退、不可变证据、终态和必须原子完成的一次性消费/撤销安全边界；不得依赖数据库触发器实现第二套完整领域流程。
- 状态更新必须比较 expected state 与 `row_version`；需要串行分配版本、epoch 或单次结果时锁定聚合根。领域写入、撤销、Outbox、Audit Outbox 和幂等结果在同一事务提交。
- 具体上下文匹配字段、Canonical JSON 摘要、SqlSugar 事务模板、错误码和并发测试见 [`database/.NET业务规则与状态机实现清单.md`](./database/.NET业务规则与状态机实现清单.md)。
